...
function BackUp()
{
	...
	do
		if [ $port == "10010" ]; then
			...
			index=${port: -1}
			mv /data/projects/redis-$port/db/appendonly.aof /data/projects/redis-$port/db/appendonly.aof.bak; 
		fi
	done
...
function RollBack()
{
	...
	do
		if [ $port == "10010" ]; then
			...
			index=${port: -1}
			mv /data/projects/redis-$port/db/appendonly.aof.bak /data/projects/redis-$port/db/appendonly.aof; 
		fi
	done
...
function CheckBackUp()
{
	...
	do
		...
		if [ $port == "10010" ]; then
			...
			index=${port: -1}
			cnt=$(ls -lrht /data/projects/redis-$port/db/appendonly.aof.bak | wc -l);
		fi
		if [ $cnt -lt 1 ]; then
			echo "error /data/projects/redis-$port/db/appendonly.aof.bak no exist. Please check it!"
		fi
	done
	echo "End BackUp DmpRedisAof ... "
}
...
function CheckRollBack()
{
	...
	do
		...
		if [ $port == "10010" ]; then
			...
			index=${port: -1}
			cnt=$(ls -lrht /data/projects/redis-$port/db/appendonly.aof | wc -l)
		fi
		if [ $cnt -lt 1 ]; then
			echo "error /data/projects/redis-$port/db/appendonly.aof no exist. Please check it!"
		fi
	done
